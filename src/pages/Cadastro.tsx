...
import { AtSign, Eye, EyeOff, LockKeyhole, Store, UserRound } from 'lucide-react'
...
export default function Cadastro() {
  ...
  async function onSubmit(e: FormEvent) {
    ...
    setCarregando(true)
    try {
      await registrar({ nome, email, senha, oficina })
      ...
      <section className="relative hidden lg:flex flex-col justify-between overflow-hidden border-r seam p-12">
        ...
        <div className="relative max-w-md">
          ...
          <p className="mt-5 text-ink-2 text-base leading-relaxed">
            Injeção leve e diesel, ABS, elétrica e câmbio. Sem instalar nada: abre no computador
            da bancada, no tablet e no celular.
          </p>
        </div>
      ...
      <section className="flex items-center justify-center p-6 sm:p-10">
        <div className="w-full max-w-[420px]">
          ...
          <div className="relative rounded-2xl border seam bg-bench-2 p-7 sm:p-8">
            ...
            <p className="mt-1.5 text-ink-3 text-sm">Leva menos de um minuto.</p>

            <form onSubmit={onSubmit} className="mt-7 space-y-4" noValidate>
              <label className="block">
                <span className="mb-1.5 block text-[13px] font-medium text-ink-2">Seu nome</span>
                <span className="relative block">
                  <UserRound size={17} className="absolute left-4 top-1/2 -translate-y-1/2 text-ink-4" />
                  <input className="field pl-11" type="text" autoComplete="name" placeholder="como querem te chamar"
              ...
              <label className="block">
                ...
                <span className="relative block">
                  ...
                  <input className="field pl-11" type="email" autoComplete="email" autoCapitalize="none"
                    autoCorrect="off" spellCheck={false} placeholder="voce@oficina.com.br"
              ...
              <label className="block">
                <span className="mb-1.5 block text-[13px] font-medium text-ink-2">
                  Oficina <span className="font-normal text-ink-4">(opcional)</span>
                </span>
                <span className="relative block">
                  <Store size={17} className="absolute left-4 top-1/2 -translate-y-1/2 text-ink-4" />
                  <input className="field pl-11" type="text" autoComplete="organization" placeholder="nome da oficina"
                    value={oficina} onChange={(e) => setOficina(e.target.value)} />
                </span>
              </label>

              <label className="block">
                <span className="mb-1.5 block text-[13px] font-medium text-ink-2">Senha</span>
                <span className="relative block">
